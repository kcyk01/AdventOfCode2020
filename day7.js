const input = `shiny lime bags contain 3 muted magenta bags, 3 clear cyan bags.
...
const rules = input
  .split('\n')
  .map(r => r.match(/^(.+) bags contain (.+)\.$/))
  .reduce((d, r) => {
    const parent = r[1];
    const children = r[2].split(',').map(r => r.trim().match(/(no|\d+) (.+) bags?/).slice(1, 3));
    d.p2c[parent] = children;
...
const part1 = () => {
  const acceptableBags = new Set();
  let queue = rules.c2p['shiny gold'];
  while (queue.length) {
    let next = queue.shift();
    if (acceptableBags.has(next)) {
      continue;
    }
    acceptableBags.add(next);

    if (rules.c2p[next]) {
      queue = [...queue, ...rules.c2p[next].filter(b => !acceptableBags.has(b))];
    }
  }
  return acceptableBags.size;
};

const part2 = () => {
  let queue = rules.c2p['other'];
  const checked = new Set(queue);
  const bagCount = queue.reduce((d, b) => {
    d[b] = 1;
    return d;
  }, {});
  ...
  while (queue.length) {
    let next = queue.shift();
    const parentBags = Object.keys(rules.p2c)
      .filter(k => !checked.has(k) // Ignore already checked bags
                && rules.p2c[k].some(c => c[1] === next) // Only look at parent bags of the current bag
                && rules.p2c[k].every(c => checked.has(c[1]))); // Make sure all children bags of the parent is processed
    
    parentBags.forEach(k => {
      rules.p2c[k].forEach(sub => {
        if (!bagCount[k]) {
          bagCount[k] = 1;
        }
        bagCount[k] += parseInt(sub[0]) * bagCount[sub[1]];
        queue.push(k);
      });
      checked.add(k);
    });

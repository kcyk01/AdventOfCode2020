...
const part1 = () => {
  ...
  for (let passport of passports) {
    if (Object.keys(passport).filter(f => requiredFields.has(f)).length === requiredFields.size) {
      valid++;
    }
  }
  return valid;
...
const part2 = () => {
  ...
  for (let passport of passports) {
    ...
    const hairColor = passport['hcl'].match(/^\#[0-9a-f]{6}$/);
    if (!hairColor) {
      continue;
    }
    
    const eyeColor = passport['ecl'].match(/^amb|blu|brn|gry|grn|hzl|oth$/);
    if (!eyeColor) {
      continue;
    }

    const passportId = passport['pid'].match(/^\d{9}$/);
    if (!passportId) {
      continue;
    }

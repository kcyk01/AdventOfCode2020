...
const policies = input.split('\n').map(p => p.match(/^(\d+)-(\d+) (\w): (\w+)$/));

const part1 = () => {
  let count = 0;
  for (let policy of policies) {
    const letterCount = policy[4].match(new RegExp(policy[3], 'g'));
    const occurences = letterCount && letterCount.length || 0;
    if (occurences >= parseInt(policy[1]) && occurences <= parseInt(policy[2])) {
      count++;
    }
  }
  return count;
...
const part2 = () => {
  let count = 0;
  for (let policy of policies) {
    const char1 = policy[4][parseInt(policy[1])-1];
    const char2 = policy[4][parseInt(policy[2])-1];
    if (char1 === policy[3] ^ char2 === policy[3]) {
      count++;
    }
  }
  return count;
